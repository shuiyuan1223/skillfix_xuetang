...
import { getFromMemoryCache, saveToMemoryCache, clearMemoryCache } from '../../src/data-sources/huawei/api-cache.js';
...
describe('Memory Cache', () => {
  ...
  describe('saveToMemoryCache and getFromMemoryCache', () => {
    ...
    test('returns null when TTL expired', async () => {
      ...
      getFromMemoryCache('test-endpoint', { id: 1 }, 1); // 1ms TTL
  ...
  describe('clearMemoryCache', () => {
    test('clears all cached data', () => {
      ...
      expect(getFromMemoryCache('a', { x: 1 })).toBeUndefined();
      expect(getFromMemoryCache('b', { x: 2 })).toBeUndefined();
    });
  });
