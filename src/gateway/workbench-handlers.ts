...
async function doRunDiffInterpret(session: GatewaySession, result: WorkbenchDiffResult): Promise<void> {
  ...
  const diffSummaryInput = [
    ...
    '',
    '判断规则：',
    '- 【单点修改】：修改集中在某一具体规则/场景/知识点（如新增一个因素、调整某个阈值、补充一个示例）',
    '  → 必须在 Before/After 中精确找到受影响的句子，用 **粗体** 标注，不标注无关内容',
    '- 【框架级修改】：修改涉及整体结构、主要流程、多个核心规则（如重写大段、调整整体框架）',
    '  → 在 SUMMARY 中注明”框架级修改，多处内容受影响”，仍尽量标注最核心的变化句子（选取差异最大的3-5处）',
    '',
    '要求：',
    '- 你的输出必须且只能包含下面三个标记段落，不要在标记之前或之外输出任何文字',
    '- 标注格式：用 **关键句** 包裹受影响文字，其余内容原样保留，不要改写或缩减',
    '- 如果某句在 Before 和 After 完全相同，则不标注',
    '',
    '【变更 Diff（仅启用项）】',
    ...promptDiffs
      .filter((d) => d.enabled)
    ...
    '',
    '<<<SUMMARY>>>',
    '（2-3句：说明是单点修改还是框架级修改，改变了什么规则，对输出产生了什么影响）',
    '<<<ANNOTATED_BEFORE>>>',
    '（Before 输出原文完整复制，将受影响的关键句用 **关键句** 包裹，其余原样保留）',
    '<<<ANNOTATED_AFTER>>>',
