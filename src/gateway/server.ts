...
export class GatewaySession {
  ...
  private buildEvolutionLabData(): EvolutionLabData {
    ...
    return {
      ...
      ...(activeTab === "incidents" ? this._loadIncidentsTabData() : {}),
    ...
    try {
      ...
      if (runs.length > 0) {
        try {
          ...
          if (ls.length > 0) latestRunCategoryScores = parseAndReAggregateScores(ls);
        } catch {
          /* ok */
        }
      }
  ...
  private _loadOverviewTabData(): Pick<
    EvolutionLabData,
    "stats" | "scoreTrend" | "versionCount" | "testCaseCount"
  > {
  ...
  private _loadVersionsTabData(
    ...
    try {
      ...
      for (const v of versionRows) timelineEvents.push(mapVersionToTimelineEvent(v));
    } catch {
  ...
  private _loadDataTabData(): Pick<
    EvolutionLabData,
    "traces" | "tracesTotal" | "evaluations" | "suggestions"
  > {
...
function mapBenchmarkRun(
  ...
  if (r.metadata) {
    try {
      ...
      presetName =
        rawPreset && benchmarkModelsCfg[rawPreset]?.label
          ? benchmarkModelsCfg[rawPreset].label
          : rawPreset;
    } catch {
      /* ok */
    }
