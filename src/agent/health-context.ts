...
async function fetchAllHealthData(source: HealthDataSource, today: string, weekStartStr: string): Promise<any[]> {
  return Promise.all([
...
function buildSleepSection(weeklySleep: Array<{ date: string; hours: number }>): string | null {
  ...
  const avgSleep = Math.round((sleepDays.reduce((s, d) => s + d.hours, 0) / sleepDays.length) * 10) / 10;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function buildVitalsSection(todayHR: any, todayStress: any, todaySpO2: any, todayBP: any, todayGlucose: any, todayTemp: any): string[] {
  const sections: string[] = [];
  if (todayHR) {
    sections.push(`**Heart Rate (today)**: Resting avg ${todayHR.restingAvg} bpm, range ${todayHR.minToday}-${todayHR.maxToday} bpm`);
  }
  if (todayStress) {
    sections.push(`**Stress (today)**: Current ${todayStress.current}, avg ${todayStress.avg}, range ${todayStress.min}-${todayStress.max}`);
  }
  if (todaySpO2) {
    sections.push(`**SpO2 (today)**: Current ${todaySpO2.current}%, avg ${todaySpO2.avg}%, range ${todaySpO2.min}-${todaySpO2.max}%`);
  }
  if (todayBP) {
    sections.push(`**Blood Pressure (today)**: ${todayBP.latestSystolic}/${todayBP.latestDiastolic} mmHg`);
  }
  if (todayGlucose) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function collectInsights(weeklySteps: any[], weeklySleep: any[], todayHR: any, todayStress: any, todaySpO2: any, todayBP: any, todayGlucose: any, todayTemp: any): string[] {
  const insights: string[] = [];
  const sleepDays = weeklySleep.filter((d: { hours: number }) => d.hours > 0);

  if (sleepDays.length >= 5) {
    const avgSleep = sleepDays.reduce((s: number, d: { hours: number }) => s + d.hours, 0) / sleepDays.length;
    if (avgSleep < 6) insights.push("Sleep duration has been consistently below 6 hours — worth discussing.");
  }
  ...
  if (todayHR && todayHR.restingAvg > 90) insights.push("Today's resting heart rate is elevated (>90 bpm).");
  if (todayStress && todayStress.avg > 60) insights.push("Today's average stress level is high (>60).");
  if (todaySpO2 && todaySpO2.avg < 95) insights.push("Today's blood oxygen is below normal (<95%).");
  if (todayBP && (todayBP.latestSystolic > 140 || todayBP.latestDiastolic > 90)) {
    insights.push("Blood pressure is elevated (>140/90 mmHg).");
  }
  if (todayGlucose && todayGlucose.latest > 7.0) insights.push("Blood glucose is elevated (>7.0 mmol/L fasting).");
  if (todayTemp && todayTemp.latest > 37.5) insights.push("Body temperature is elevated (>37.5\u00B0C).");
  return insights;
}

function buildPlansSection(userUuid: string | undefined, today: string, data: HealthSnapshot): string {
  try {
    ...
    for (const plan of activePlans) {
      ...
      for (const goal of plan.goals) {
        ...
        const arrow = goal.status === "completed" || goal.status === "ahead" ? "\u2191" :
          goal.status === "behind" ? "\u2193" : "\u2192";
...
function buildProactiveSection(userUuid: string | undefined): string {
  try {
    ...
    const upcomingEvents = listCalendarEvents(uuid, { from: now, to: weekLater, status: "scheduled" });
    ...
    if (activeRecs.length > 0) {
      ...
      for (const rec of activeRecs.slice(0, 5)) result += `\n- [${rec.priority}] ${rec.title}: ${rec.body}`;
    }
...
export async function preComputeHealthContext(
  ...
  try {
    ...
    const [
      weeklySteps, weeklySleep, todayHR, todayWorkouts, weeklyWorkouts,
      todayStress, todaySpO2, todayBP, todayGlucose, todayTemp,
      todayMetrics, todayBodyComp,
    ...
    sections.push(...buildVitalsSection(todayHR, todayStress, todaySpO2, todayBP, todayGlucose, todayTemp));
    sections.push(buildWorkoutSection(todayWorkouts));

    if (sections.length === 0) return "";

    const insights = collectInsights(weeklySteps, weeklySleep, todayHR, todayStress, todaySpO2, todayBP, todayGlucose, todayTemp);
    const dateRange = weeklySteps.length > 0
    ...
    const snapshot: HealthSnapshot = { weeklySteps, weeklySleep, todayHR, todayWorkouts, weeklyWorkouts, todayMetrics, todayBodyComp };
...
function resolveMetricValue(
  ...
  switch (metric) {
    case "steps": return resolveSteps(frequency, data);
    case "sleep_hours": return resolveSleepHours(frequency, data);
    case "exercise_count": return frequency === "weekly" ? data.weeklyWorkouts.length : data.todayWorkouts.length;
    case "heart_rate_resting": return data.todayHR?.restingAvg ?? null;
    case "weight": return data.todayBodyComp?.weight ?? null;
    case "calories": return data.todayMetrics?.calories ?? null;
    case "active_minutes": return data.todayMetrics?.activeMinutes ?? null;
    default: return null;
  }
}
