...
import type { HealthDataSource, HealthMetrics, BodyCompositionData } from '../data-sources/interface.js';
...
function buildActivitySection(weeklySteps: Array<{ date: string; steps: number }>): string | null {
  ...
  if (weeklySteps.length >= 6) {
    ...
    if (earlierAvg > 0) {
      ...
      if (pct > 10) {
        trend = ` (trending up ~${pct}%)`;
      } else if (pct < -10) {
        trend = ` (trending down ~${Math.abs(pct)}%)`;
      } else {
        trend = ' (stable)';
      }
    }
...
function buildSleepSection(weeklySleep: Array<{ date: string; hours: number }>): string | null {
  ...
  if (spread <= 1) {
    ...
    consistency = `, inconsistent (${minSleep}h-${maxSleep}h range)`;
  }
...
export async function preComputeHealthContext(dataSource?: HealthDataSource, userUuid?: string): Promise<string> {
  try {
    ...
    const dateRange =
      weeklySteps.length > 0 ? `${weeklySteps[0].date} to ${weeklySteps[weeklySteps.length - 1].date}` : `past 7 days`;
...
export async function fetchWeatherContext(userUuid?: string): Promise<string> {
  try {
    ...
    if (!location) {
      location = contextConfig?.location;
    }
...
export function autoSyncPlanProgress(plans: HealthPlan[], uuid: string, today: string, data: HealthSnapshot): void {
...
function resolveMetricValue(metric: string, frequency: 'daily' | 'weekly', data: HealthSnapshot): number | null {
  switch (metric) {
    case 'steps':
