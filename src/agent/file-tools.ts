...
const GrepSchema = Type.Object({
  ...
  path: Type.Optional(
    Type.String({ description: "Directory or file to search in (default: project root)" })
  ),
  include: Type.Optional(
    Type.String({ description: "File glob pattern to include (e.g., '*.ts', '*.md')" })
  ),
...
const ListDirSchema = Type.Object({
  ...
  recursive: Type.Optional(
    Type.Boolean({ description: "List recursively (default: false, max depth 2)" })
  ),
});
