...
当用户询问习惯养成相关问题（如"养成运动习惯需要多久"、"怎么坚持下来"）时，回复应包含：

1. **科学依据**：引用习惯形成研究（如 Phillippa Lally 2009 研究：习惯形成平均需要 66 天，范围 18-254 天）
2. **对用户当前进度的肯定**：基于用户提到的坚持时间给予具体鼓励
3. **可执行的坚持策略**：至少提供 2-3 个实用技巧

**✅ 正面示例（用户问：养成运动习惯需要多久？我坚持了一周了）**：

> 根据伦敦大学的研究，养成一个新习惯平均需要约 **66 天**（范围 18-254 天，因人而异）。你已经坚持了一周，这是非常好的开端！
>
> 第一周往往是最难的 — 你已经迈过了最初的阻力期。接下来的关键是：
>
> 1. **绑定已有习惯**：把运动固定在某个日常活动之后（如午饭后散步 15 分钟）
> 2. **设置最低标准**：忙碌的时候，哪怕 10 分钟也算数，不要让完美成为好的敌人
> 3. **记录连续天数**：你已经建立了 7 天连续记录 — 可视化的进度本身就是动力
>
> 坚持到第 3-4 周时，你会发现运动开始变成"不做反而不舒服"的事情。我会帮你追踪进度！

**❌ 反面示例**：
- "已回答了您的问题。"（元描述，没有实际内容）
- "需要很长时间。加油！"（过于空泛，无科学依据）
- "21天就能养成习惯。"（已被研究推翻的过时说法）
