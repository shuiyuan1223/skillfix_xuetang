...
export const getWeeklySummaryTool: PHATool<{}> = {
  ...
  execute: async () => {
    ...
    const source = getDataSource();
    const [weeklySteps, weeklySleep] = await Promise.all([source.getWeeklySteps(today), source.getWeeklySleep(today)]);
    ...
    const avgSleep = sleepDays.length > 0 ? Math.round((totalSleep / sleepDays.length) * 10) / 10 : 0;
...
export const getBloodGlucoseTool: PHATool<{ date: string }> = {
  ...
  description: '获取指定日期的血糖数据。返回血糖读数（mmol/L），含平均值、最高值、最低值。当用户询问血糖时调用。',
  displayName: '血糖数据',
  category: 'health' as const,
  icon: 'flame',
...
export const getBodyTemperatureTool: PHATool<{ date: string }> = {
  ...
  description: '获取指定日期的体温数据。返回体温读数（摄氏度），含平均值、最高值、最低值。当用户询问体温或发烧时调用。',
  displayName: '体温数据',
  category: 'health' as const,
  icon: 'stethoscope',
  sessionBound: true,
...
export const getEmotionTool: PHATool<{ date: string }> = {
  name: 'get_emotion',
  description: '获取指定日期的情绪数据。返回当前情绪状态、心情评分及全天读数。当用户询问情绪或心理状态时调用。',
  displayName: '情绪数据',
  category: 'health' as const,
  icon: 'sparkles',
  companionSkill: 'stress-management',
...
function makeSingleExecute(
  ...
  return async (args: { date: string }) => {
    const date = args.date === 'today' ? new Date().toISOString().split('T')[0] : args.date;
    if (!getter) {
      return { success: true, data: null, message: unsupportedMsg };
    }
...
function buildCoreHealthTools(source: HealthDataSource) {
  return {
    getHealthData: {
      ...getHealthDataTool,
      execute: makeRangeExecute(source, source.getMetricsRange?.bind(source), (d) => source.getMetrics(d)),
    },
    getHeartRate: {
      ...getHeartRateTool,
      execute: makeRangeExecute(source, source.getHeartRateRange?.bind(source), (d) => source.getHeartRate(d)),
    },
    getSleep: {
      ...getSleepTool,
...
function buildExtendedHealthTools(source: HealthDataSource) {
  return {
    ...
    getHRV: {
      ...getHRVTool,
      execute: makeSingleExecute(source.getHRV?.bind(source), 'HRV data not supported.', 'No HRV data available.'),
    },
  };
}
