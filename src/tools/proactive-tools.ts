...
const createRecommendationTool: PHATool<CreateRecommendationParams> = {
  ...
  description: '创建主动健康建议/推荐（类似小艺建议）。适用于根据用户健康数据主动推送有针对性的建议。',
  displayName: '健康推荐',
  category: 'proactive',
  icon: 'sparkles',
  ...
  execute: async (params: CreateRecommendationParams) => {
    ...
    const rec: Recommendation = {
      ...
      icon: params.icon,
      action:
        params.actionLabel && params.actionId ? { label: params.actionLabel, action: params.actionId } : undefined,
...
const dismissRecommendationTool: PHATool<DismissRecommendationParams> = {
  ...
  execute: async (params: DismissRecommendationParams) => {
    ...
    if (!rec) {
      return { error: 'Recommendation not found', id: params.id };
    }
...
const createReminderTool: PHATool<CreateReminderParams> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      category: {
        type: 'string',
        description: 'Category: medication, exercise, sleep, hydration, meal, checkup, custom (default: custom)',
      },
...
const completeReminderTool: PHATool<CompleteReminderParams> = {
  ...
  execute: async (params: CompleteReminderParams) => {
    ...
    if (!reminder) {
      return { error: 'Reminder not found', id: params.id };
    }
...
const createCalendarEventTool: PHATool<CreateCalendarEventParams> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      category: {
        type: 'string',
        description: 'Category: workout, checkup, meal, sleep, medication, custom (default: custom)',
      },
      icon: { type: 'string', description: 'Icon name' },
      relatedPlanId: { type: 'string', description: 'Related plan ID' },
    },
...
const updateCalendarEventTool: PHATool<UpdateCalendarEventParams> = {
  ...
  execute: async (params: UpdateCalendarEventParams) => {
    ...
    if (params.status) {
      event.status = params.status as CalendarEvent['status'];
    }
