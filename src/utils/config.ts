...
function migrateOrchestratorFields(config: PHAConfig): boolean {
  ...
  if (!config.agentModel && !config.systemAgentModel &&
      !config.embeddingModel && typeof config.judgeModel !== "string") {
    return false;
  }
  config.orchestrator = {
...
function buildModelRepository(config: PHAConfig): Record<string, ModelProviderConfig> {
  ...
  ensureMigrationProvider(providers, llmProvider, config.llm.apiKey,
