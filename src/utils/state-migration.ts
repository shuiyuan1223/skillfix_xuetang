...
import { existsSync, mkdirSync, renameSync, unlinkSync, rmSync, writeFileSync, readdirSync, statSync } from 'fs';
...
function cleanupGarbageUserDirs(usersDir: string): void {
  ...
  try {
    for (const entry of readdirSync(usersDir)) {
      if (!GARBAGE_USER_DIRS.has(entry)) {
        continue;
      }
...
export function migrateStateDir(): void {
  ...
  // Move databases
  moveDbFile(join(projectRoot, 'data', 'pha.db'), join(dbDir, 'evolution.db'), 'data/pha.db → .pha/db/evolution.db');
