...
import { loadConfig, PROVIDER_CONFIGS, resolveAgentModel, type LLMProvider } from '../utils/config.js';
...
function handleToolStart(event: { toolName: string; args: unknown }, options: ChatOptions, state: ChatState): void {
...
function printChatResult(options: ChatOptions, state: ChatState): void {
  if (options.json) {
    console.log(
      JSON.stringify({ message: options.message, response: state.response, toolCalls: state.toolCalls }, null, 2)
    );
    return;
  }
  ...
  program
    ...
    .action(async (options) => {
      ...
      try {
        ...
        fatal('No API key found', err instanceof Error ? err.message : 'Set an API key in config or environment');
      }
