...
function handleStateInit(options: { remote?: string }): void {
  ...
  if (!options.remote) {
    console.log(`  ${icons.arrow} Add a remote: ${c.cyan('pha state init --remote <your-private-repo>')}`);
  }
...
function handleStatePull(options: { force?: boolean }): void {
  ...
  if (!hasRemote(stateDir)) {
    console.log(`${c.red(icons.error)} No remote configured. Run ${c.cyan('pha state init --remote <url>')} first.`);
    process.exit(1);
  }
...
export function registerStateCommand(program: Command): void {
  const state = program.command('state').description('Manage .pha/ state directory as a git repository');

  state
    .command('init')
    .description('Initialize .pha/ as a git repository (with --remote: reset local to match remote)')
