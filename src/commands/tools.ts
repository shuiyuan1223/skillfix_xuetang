...
import { printHeader, printSection, printTable, c, icons, truncate, Spinner, fatal } from '../utils/cli-ui.js';
  ...
  if (options.verbose) {
    ...
    printTable(
      ...
      tools.map((tool) => {
        ...
        return [c.cyan(tool.name), truncate(tool.description, 40), paramCount > 0 ? String(paramCount) : c.dim('-')];
      })
    );
    console.log('');
...
async function handleToolsCall(name: string, options: { arg?: string[]; json?: boolean }): Promise<void> {
  const tools = await mcpHandler.listTools();
  ...
  try {
    ...
    if (!options.json) {
      spinner.stop(result.isError ? 'error' : 'success');
    }
...
export function registerToolsCommand(program: Command): void {
  ...
  toolsCmd.command('info <name>').description('Show detailed info about a tool').action(handleToolsInfo);
}
