...
export function registerAuthCommand(program: Command): void {
  const auth = program.command('auth').description('Authenticate with health data provider (via Chrome MCP)');
  ...
  auth
    ...
    .action(async (options) => {
      ...
      if (config.dataSources.type === 'mock') {
        ...
        console.log(`  ${c.dim("Switch to 'huawei' via")} ${c.cyan('pha onboard')} ${c.dim('first.')}`);
        console.log('');
        return;
      }
