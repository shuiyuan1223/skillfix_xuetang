...
export function registerGatewayCommand(program: Command): void {
  const gatewayCmd = program.command('gateway').description('Manage the PHA gateway server (alias for start/stop)');
  ...
  gatewayCmd.command('health').description('Check gateway health endpoint').action(handleGatewayHealth);
}
