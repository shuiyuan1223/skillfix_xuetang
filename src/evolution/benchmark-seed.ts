...
import type { TestCase, CategoryWeightConfig, BenchmarkCategory, SharpRubricCategory } from './types.js';
...
const RUBRIC_FILE = join(process.cwd(), 'src', 'skills', 'benchmark-evaluator', 'reference', 'sharp_rubrics.json');
  ...
  try {
    ...
    if (metaStart >= 0) {
      ...
      for (const line of afterMeta.split('\n')) {
        ...
        if (lines.length > 0 && !line.startsWith('  ') && line.trim() !== '') {
          break;
        }
        lines.push(line);
      }
 ...
 */
export function getBenchmarkTests(options: { profile?: 'quick' | 'full'; category?: BenchmarkCategory }): TestCase[] {
