...
export class Analyzer {
  ...
  private identifyWeaknesses(evaluations: EvaluationResult[]): AnalysisResult["weaknesses"] {
    ...
    for (const category of categories) {
      ...
      if (avgScore < 70) {
        ...
        if (avgScore < 50) impact = "high";
        else if (avgScore < 60) impact = "medium";
  ...
  private getSuggestedFix(category: string, _score: number): string {
    const fixes: Record<string, string> = {
      ...
      helpfulness:
        "Make responses more actionable. Include specific, concrete suggestions rather than general advice.",
      safety:
